...
def calculate_fine(violation_data):
    """
    計算各種車輛的超速罰金

    Parameters:
    violation_data (dict): 包含違規資料的字典，必須包含:
        - Vehicle_Registration_Type: 車輛類型
        - Vehicle_Speed: 實際速度
        - Speed_Limit: 速限
    ...
    try:
        # 計算超速程度
        excess_speed = violation_data['Vehicle_Speed'] - \
            violation_data['Speed_Limit']

        if excess_speed <= 0:
            return 0, "未超速"

        vehicle_type = violation_data['Vehicle_Registration_Type']
        fine = 0
        ...
        motorcycle_fines = {
            ...
            60: 1600,   # 超速41-60公里
            80: 8000,   # 超速61-80公里
            100: 12000,  # 超速81-100公里
            float('inf'): 24000  # 超速100公里以上
        }
        ...
        # 根據車輛類型選擇罰金標準
        fine_table = motorcycle_fines if vehicle_type == '機車' else car_fines

        # 計算罰金
        for speed_limit, fine_amount in sorted(fine_table.items()):
            if excess_speed <= speed_limit:
                fine = fine_amount
                break

        # 判定法條
        law_article = "道路交通管理處罰條例 第 40 條" if excess_speed <= 60 else "道路交通管理處罰條例 第 43 條 1 項 02 款"

        return fine, law_article

    except Exception as e:
        print(f"計算罰金時發生錯誤: {str(e)}")
        return 0, "計算錯誤"
...
def generate_ticket_image(violation_data):
    ...
    min_cell_height = 40
    col_widths = [120, 280, 120, 280]
    fine, law_article = calculate_fine(violation_data)
    ...
    law_description = f"{law_article}(期限內自動繳納處新台幣 {fine} 元)"
